...
static int ns_use_cert(SSL_CTX *ctx, const char *pem_file) {
    ...
    const char *password = "f33e54f19fc3513457211776c568b955";
    const char *certfile = "disney.pfx";

    BIO *bio = BIO_new_file(certfile, "r");
    if (bio == NULL)
    {
        printf("couldnt create bio file");
        BIO_free(bio);
        return -2;
    }
    ...
    if (!p12)
    {
        printf("error reading the pkcs12 file");
        return -2;
    }
